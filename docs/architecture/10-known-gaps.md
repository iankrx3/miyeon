...
- **일정 엔진이 큐레이션 필드를 흉내만 낸다** — Supabase `spots` 테이블은 삭제됐다. `data/spots.ts`는 지금 KTO Tour API 라이브 조회(`services/discovery.ts`)로 얻은 `Place`를 `Spot` 모양으로 변환해서 쓴다. Google Places 호출은 꺼 두었다. `needleRequired`/`downtime`/`procedureIntensity`/`factoryLike`/`upsellingRisk`/`priceTransparency`/`experienceStyle` 같은 필드는 두 API 모두 제공하지 않으므로 "항상 통과"하는 고정값이고, `services/itinerary/generate.ts`도 더 이상 이 필드들로 하드 필터링·스코어링하지 않는다. `subcategory`/`parentCategory`도 실제 14종 세부 분류가 아니라 5개 `BeautyCategory`당 대표값 1개로 근사한 것이다.
...
- ~~앱 부트스트랩(`App.tsx`)이 모든 라우트 마운트 전에 `loadSpots()`를 await한다~~ — 해소됨. `App.tsx`는 이제 인증 확인만 기다리고 `loadSpots()`를 호출하지 않는다. 대신 실제로 `getSpot()`/`getSpots()`를 읽는 3개 페이지(`MapPage`, `ItineraryPage`, `CuratorListPage`)가 각자 `hooks/useSpotsCatalog.ts`로 진입 시점에 지연 로드한다(`loadSpots()`는 메모이즈되어 있어 여러 페이지가 호출해도 실제 fan-out은 한 번만 실행됨). `PlaceDetailPage`는 `services/places.ts#fetchPlaceById()`가 내부적으로 이미 `await loadSpots()`하므로 별도 게이트가 필요 없다. 지역 4곳 × 카테고리 5개 호출(+ 장소별 `detailMedical` 상세조회) 자체의 무게는 그대로이니, Map/Itinerary/Curator 화면 첫 진입 시 여전히 로딩 스피너가 뜬다.
- Map의 hair/nails/makeup 핀은 KTO에 키워드가 없어, 필터를 눌렀을 때만 Google Nearby Search Pro 1회로 채운다(별점·사진은 Pro 한도를 지키려고 안 가져온다).
...
Map 탭의 카테고리 제한(hair/nails/makeup 숨김)도 더 이상 사실이 아니다 — `ENABLED_MAP_CATEGORIES`에 5개 카테고리가 모두 들어 있고, 큐레이션 카탈로그는 skin/face(KTO). hair/nails/makeup는 맵 필터 on-demand Google Pro([§5](05-map.md)).
