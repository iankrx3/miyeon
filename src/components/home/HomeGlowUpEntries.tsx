...
export const HomeGlowUpEntries: React.FC<HomeGlowUpEntriesProps> = ({ onStartAnalysis }) => (
  <div className="bg-white pb-7 pt-[30px]">
    <div className="mx-auto max-w-5xl px-5 sm:px-8">
      ...
      <p className="mt-1 font-display text-[21px] font-medium text-miyeon-ink sm:text-2xl">We Make It Yours</p>
    </div>
      ...
      {pillars.map((pillar) => (
        ...
        >
          ...
          <div className="absolute bottom-3.5 left-3.5 right-3.5">
            <p className="font-display text-[16px] font-bold tracking-[0.5px] text-white">{pillar.label}</p>
            <p className="mt-[3px] text-[11px] text-white/80">{pillar.meta}</p>
          </div>
        </motion.button>
      ))}
    </div>
