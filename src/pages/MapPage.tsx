...
import type { BeautyCategory, Place, UserSession } from '../types';

const FILTERS: { id: 'all' | BeautyCategory; label: string }[] = [
  { id: 'all', label: 'All' },
  ...Object.entries(categoryMeta).map(([id, meta]) => ({ id: id as BeautyCategory, label: meta.label })),
];
...
export default function MapPage({ session, onSignIn }: { session: UserSession; onSignIn: () => void }) {
  ...
  const [category, setCategory] = useState<'all' | BeautyCategory>('all');
  ...
  return (
    <div className="flex h-[calc(100dvh-64px)] flex-col">
      <div className="shrink-0 bg-gradient-to-b from-[#f9dde4] to-[#fef6f8] px-5 pb-5 pt-5">
        ...
        <p className="mt-1.5 text-[12.5px] text-miyeon-main/65">Real routes from people who actually went.</p>
      </div>

      <div className="flex shrink-0 gap-1.5 overflow-x-auto bg-white px-5 py-3 no-scrollbar">
        {FILTERS.map((f) => (
          <button
            key={f.id}
            type="button"
            ...
            className={`shrink-0 rounded-full px-3.5 py-1.5 text-xs font-medium ${
              category === f.id ? 'bg-miyeon-ink text-white' : 'bg-miyeon-surface text-miyeon-main/70'
            }`}
          >
            {f.label}
          </button>
      ...
      <div className="relative min-h-0 flex-1">
        {!spotsReady ? (
          <div className="flex h-full w-full items-center justify-center bg-miyeon-surface">
          ...
          <>
            ...
            <div className={viewMode === 'list' ? 'h-full' : 'hidden'}>
        ...
        <motion.button
          ...
          className="absolute left-3 bottom-[calc(var(--bottom-nav-h)+20px)] z-20 flex h-10 w-10 items-center justify-center rounded-xl bg-white text-miyeon-ink shadow-lg border border-black/5 hover:text-miyeon-accent sm:bottom-5"
        >
        ...
        {!(viewMode === 'map' && selectedPlace) && (
          <div className="absolute left-1/2 z-20 -translate-x-1/2 bottom-[calc(var(--bottom-nav-h)+20px)] sm:bottom-5">
            <motion.button
              ...
              className="flex items-center gap-1.5 rounded-full bg-miyeon-ink px-4 py-2.5 text-xs font-medium text-white shadow-lg"
            >
            ...
            <div className="absolute bottom-[calc(var(--bottom-nav-h)+16px)] left-1/2 z-30 w-[min(92vw,420px)] -translate-x-1/2 sm:bottom-4">
              ...
              >
                <button
                  ...
                  className="absolute right-3 top-3 text-miyeon-main/50 hover:text-miyeon-ink"
                >
                  <X className="h-4 w-4" />
                </button>
                <p className="pr-6 text-sm font-medium text-miyeon-ink">{selectedPlace.name}</p>
                <p className="mt-0.5 flex items-center gap-1 text-xs text-miyeon-main/60">
                  <Star className="h-3 w-3 fill-miyeon-accent-dark text-miyeon-accent-dark" /> {selectedPlace.rating} · {selectedPlace.priceRange} · {selectedPlace.area}
                </p>
                <div className="mt-3 flex gap-2">
                  <motion.button
                    ...
                    className="flex-1 rounded-full bg-miyeon-ink px-4 py-2.5 text-xs font-medium text-white"
                  >
                    View place
                  </motion.button>
                  <motion.button
                    ...
                    className={`flex items-center justify-center gap-1.5 rounded-full px-4 py-2.5 text-xs font-medium ${
                      isSaved(selectedPlace.id) ? 'bg-miyeon-accent text-white' : 'bg-miyeon-surface text-miyeon-ink'
                    }`}
                  >
